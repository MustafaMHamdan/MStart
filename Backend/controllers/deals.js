...
const getMyDeals = (req, res) => {
  ...
  const getDealQuery = `SELECT ClaimedDeals.ID, Name  ,Description,Photo ,ClaimedDeals.Amount , ClaimedDeals.Currency ,User_ID FROM ClaimedDeals inner join deals on ClaimedDeals.Deal_ID=Deals.ID  WHERE User_ID = ?;`;
